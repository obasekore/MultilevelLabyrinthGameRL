...
class LabyrinthCtrlEnv(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self, headless=False, historical_samples = 3, historical_axis=1, level = 0, verbose = 1):
        self.action_space = gym.spaces.box.Box(low=np.array([-1.0, -1.0]), high=np.array([1.0, 1.0])) # (sin(roll), 
...
#   
    ...
    def step(self, action):
        ...
        target_roll = np.clip((roll_angle + action[0]), self.scene_parameter['scene-input-min-max'][0], self.scene_parameter['scene-input-min-max'][1])
        target_pitch = np.clip((pitch_angle + action[1]), self.scene_parameter['scene-input-min-max'][0], self.scene_parameter['scene-input-min-max'][1])
        ...
        self.obs = self.set_obs_history(self.obs, ob, axis = self.axis)
...
#         reward += 1.0/max(self.prev_dist_to_goal - dist_to_goal, 1)
        ...
        if (self.prev_dist_to_goal == dist_to_goal):
            self.local_stuck += 1
            if self.local_stuck>500: # terminate if stuck for 500 steps
                self.done = True
                reward -= 100
                self.local_stuck = 0
    ...
    def reset(self):
        ...
        if (self.verbose == 1) and (self.wall is not None) and len(self.rec_video)>4:
            imageio.mimsave('{}labyrinthGame{}.gif'.format(self.log_location,time.time()), [np.array(img) for i, img in enumerate(self.rec_video) if i%2 == 0], fps=5)
            self.rec_video = []
        ...
        self.ball_object = Shape.create(type=PrimitiveShape.SPHERE, 
                    ...
                    position=startDummy.get_position(), mass=0.001)
